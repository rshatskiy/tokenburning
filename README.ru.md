...
**Сколько ты на самом деле выжал из своих ИИ-подписок.**

$100 за подписку → токенов на $4000 по ценам API. tokenburning читает локальные логи Claude Code, Codex, Cursor, Gemini CLI, GitHub Copilot, OpenCode и семейства Cline, считает по живым ценам и показывает твой коэффициент выжимания (**×N к API**), куда уходит спенд и как ты на самом деле работаешь — один статический бинарь, ставится за секунды, по умолчанию ничего не шлёт в сеть.
...
Ты платишь фикс за подписку, а гоняешь токенов на кратно большую сумму по ценам API — но не видишь, насколько обыграл счётчик и где именно уходит спенд. Цифры лежат в разных локальных форматах логов, и ни один из них не сводит их для тебя.

`tokenburning` читает эти логи **локально**, считает стоимость по живым ценам и показывает в одном виде: твой коэффициент выжимания (**×N к API**), суммарный расход и куда он уходит по инструментам / моделям / проектам, и как на самом деле выглядят твои рабочие сессии. Один `curl | sh`, локальный дашборд, ничего не покидает машину.
...
Раскатал ИИ на команду? «Вроде пользуются» наверх не продашь — руководству нужно показать, что внедрение идёт и бюджет на подписки окупается. Дашборд тимлида показывает, кто реально включился (а кто оплачивает простаивающую подписку), извлечённую ценность с **ROI к бюджету подписок** и кому нужна помощь; еженедельный дайджест прилетает в Telegram. По согласию и только агрегаты — отчитаться, не превращаясь в надзирателя. Зарегистрируйся, создай организацию, пригласи разработчиков — каждый получит команду в одну строку со своим токеном:
